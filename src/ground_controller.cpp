    ...
    void run_input_loop() {
        ...
        while (rclcpp::ok()) {
            ...
            if (line.rfind("leader", 0) == 0) {
                int new_leader = std::stoi(line.substr(6));
            ...
            if (parse_line(line, nx, ny, nz)) {
                ...
                RCLCPP_WARN(this->get_logger(), "Invalid input. Use x:y:z, 'leader <id>', or commands 'land', 'arm', 'ready', 'exit'");
            }
        }
    }
...
private:
    ...
    // auto client = setmode_clients_[drone_ns];
    // if (!client->wait_for_service(5s)) {
    //     RCLCPP_ERROR(this->get_logger(), "%s SetMode service unavailable", drone_ns.c_str());
    //     return;
    // }
    ...
    //     auto response = future.get();
    //     if (response->mode_sent)
    //         RCLCPP_INFO(this->get_logger(), "%s LAND mode sent successfully", drone_ns.c_str());
    //     else
    //         RCLCPP_WARN(this->get_logger(), "%s LAND mode failed", drone_ns.c_str());
    // });
    // }
    ...
    //         auto response = future.get();
    //         if (response->mode_sent)
    //             RCLCPP_INFO(this->get_logger(), "%s OFFBOARD mode sent successfully", drone_ns.c_str());
    //         else
    //             RCLCPP_WARN(this->get_logger(), "%s OFFBOARD mode failed", drone_ns.c_str());
    //     });
    // }
    ...
    //     auto client = arm_clients_[drone_ns];
    //     if (!client->wait_for_service(5s)) {
    //         RCLCPP_ERROR(this->get_logger(), "%s Arming service unavailable", drone_ns.c_str());
    //         return;
    //     }
    ...
    //         auto response = future.get();
    //         if (response->success)
    //             RCLCPP_INFO(this->get_logger(), "%s Vehicle ARMED successfully", drone_ns.c_str());
    //         else
    //             RCLCPP_WARN(this->get_logger(), "%s ARM failed", drone_ns.c_str());
    //     });
    // }
